const { Post, User, Follower, Comment } = require('../../models/');
...
exports.addPost = async (req, res) => {
    try {
        ...
        const feedWithUser = await Post.findOne(
            {
                where: { id: feed.id },
                include: { model: User, attributes: { exclude: ["password", "createdAt", "updatedAt", "usernotif"] }, as: 'user' },
...
exports.getPostsByFollowed = async (req, res) => {
    ...
    try {
        ...
        const feed = await Post.findAll({
            ...
            include: {
                model: User,
                attributes: { exclude: ["createdAt", "updatedAt", "password", 'image', 'bio', 'usernotif'] },
                as: 'user',
            }
        });
